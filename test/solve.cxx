...
{
  using namespace durak;
  using LookUpTable = std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t> >, 4>;
  ...
  {
    auto smt = small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t>{ oneCardVsOneCard.at (0).at ({ { 0 }, { 1 } }) };
...
TEST_CASE ("nextActionsAndResults more than one action", "[abc]")
{
  ...
  gameLookup.insert ({ { 1, 1 }, solveDurak (36, 1, 1, gameLookup) });
  auto oneCardVsOneCard = std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t> >, 4>{ gameLookup.at ({ 1, 1 }) };
  SECTION ("1v1 attack draw")
  {
    auto actionsAndResults = nextActionsAndResults ({ Action{ 0 } }, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t>{ oneCardVsOneCard.at (0).at ({ { 0 }, { 4 } }) });
...
TEST_CASE ("nextActionForRole")
{
  ...
  gameLookup.insert ({ { 1, 1 }, solveDurak (36, 1, 1, gameLookup) });
  auto oneCardVsOneCard = std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t> >, 4>{ gameLookup.at ({ 1, 1 }) };
  auto actionsAndResults = nextActionsAndResults ({}, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t>{ oneCardVsOneCard.at (0).at ({ { 0 }, { 1 } }) });
