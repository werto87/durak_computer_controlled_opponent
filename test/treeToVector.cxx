...
TEST_CASE ("tuple", "[abc]")
{
  auto tree = st_tree::tree<std::tuple<uint8_t, int8_t>, st_tree::keyed<Action> >{};
  tree.insert ({ 1, 1 });
  tree.root ().insert (1, { 2, 2 });
  tree.root ().insert (2, { 3, 3 });
  tree.root ().insert (3, { 69, 69 });
  tree.root ()[0].insert (1, { 4, 4 });
  tree.root ()[0][0].insert (1, { 42, 42 });
  for (auto node : tree)
    {
      std::cout << static_cast<int> (std::get<1> (node.data ())) << std::endl;
