...
void insertGameLookUp (std::filesystem::path const &databasePath, std::map<std::tuple<uint8_t, uint8_t>, std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t> >, 4> > const &gameLookup);

small_memory_tree::SmallMemoryTree<std::tuple<Action, Result>, uint16_t> binaryToSmallMemoryTree (std::string movesAndResultAsBinary);
