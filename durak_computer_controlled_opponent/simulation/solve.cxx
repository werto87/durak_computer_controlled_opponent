...
historyEventToAction (durak::HistoryEvent const &historyEvent)
{
  if (std::holds_alternative<durak::RoundInformation> (historyEvent))
    ...
    {
      spdlog::info ("not handled HistoryEvent");
      abort ();
      return std::nullopt;
    }
}
      ...
      for (auto const &card : game.getAttackingPlayer ()->getCards ())
        {
          ...
          if (not std::get<0> (result.back ()).playerStartsAttack ({ card }))
            {
              spdlog::info ("error starting round");
              abort ();
            }
        }
    }
...
Result
searchForGameResult (std::vector<uint8_t> const &attackCardsIds, std::vector<uint8_t> const &defendCardsIds, std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result> > > const &gameResults)
{
  if (auto result = gameResults.find ({ attackCardsIds, defendCardsIds }); result != gameResults.end ())
    {
      return std::get<1> (result->second.getValues ().front ());
    }
  else
    {
...
durakInGame (Result result, durak::Game const &game)
{
  if (result == Result::AttackWon)
    ...
    {
      spdlog::info ("game should have a winner or draw");
      abort ();
    }
}
...
boost::optional<durak::Player>
calcGameResult (durak::Game const &game, std::map<std::tuple<uint8_t, uint8_t>, std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result> > >, 4> > const &gameLookup)
{
  // find game in lookup
  if (game.checkIfGameIsOver ())
  ...
  else
    {
      if (game.getAttackingPlayer () and game.getDefendingPlayer ())
          ...
          if (attackCards.size () < 2 and defendCards.size () > 1)
          ...
          else
            {
              ...
              if (gameLookup.count ({ attackCardsIds.size (), defendCardsIds.size () }) != 0)
                ...
                {
                  spdlog::info ("could not find '{}v{}' in game lookup", std::to_string (attackCardsIds.size ()), std::to_string (defendCardsIds.size ()));
                  abort ();
                }
            }
        }
        ...
        {
          spdlog::info ("game not over but no attacking or/and defending player");
          abort ();
        }
    }
  return {};
...
solveDurak (size_t n, size_t attackCardCount, size_t defendCardCount, std::map<std::tuple<uint8_t, uint8_t>, std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result> > >, 4> > const &gameLookup, std::vector<durak::Type> const &trumps)
{
  ...
  auto compressedGames = std::array<std::map<std::tuple<std::vector<uint8_t>, std::vector<uint8_t> >, SmallMemoryTree<std::tuple<Action, Result> > >, 4>{};
      ...
      for (auto trumpType : trumps)
        {
          ...
          try
            {
              auto smt = SmallMemoryTree<std::tuple<Action, Result> >{ StTreeAdapter{ convertToNonKeyedTree (tree) } };
          ...
          catch (boost::numeric::positive_overflow const &e)
            {
              ...
              spdlog::info ("exception in solveDurak.");
              spdlog::info (e.what ());
              spdlog::info ("trump: '{}'", magic_enum::enum_name (trumpType));
              ...
              spdlog::info ("attack cards: '{}'", cardsAsIdsAsString (attackCards));
              spdlog::info ("defend cards: '{}'", cardsAsIdsAsString (defendCards));
              ++skippedCombinations;
            }
        }
    }
  if (skippedCombinations) spdlog::info ("skippedCombinations: '{}'", skippedCombinations);
  return compressedGames;
}

std::vector<std::tuple<Action, Result> >
nextActionsAndResults (std::vector<Action> const &actions, small_memory_tree::SmallMemoryTree<std::tuple<Action, Result> > const &moveResults)
{
  auto const &rootElement = moveResults.getValues ().front ();
  if (auto childrenOpt = small_memory_tree::calcChildrenForPath (moveResults, { rootElement }))
    {
      ...
      std::ranges::transform (childrenOpt.value (), std::back_inserter (result),
                              [] (auto const &valueResult)
                                {
...
solveGameTree (st_tree::tree<std::tuple<Result, bool>, st_tree::keyed<Action> > &t)
{
  ...
  std::ranges::for_each (nodes.rbegin (), nodes.rend (),
                         [] (auto const &keyValue)
                           {
