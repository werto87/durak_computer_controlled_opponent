...
void
rotate_discontinuous (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, BidirIter first2, BidirIter last2, typename std::iterator_traits<BidirIter>::difference_type d2)
{
  using std::swap;
...
template <class BidirIter>
void
rotate_discontinuous3 (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, BidirIter first2, BidirIter last2, typename std::iterator_traits<BidirIter>::difference_type d2, BidirIter first3, BidirIter last3, typename std::iterator_traits<BidirIter>::difference_type d3)
{
  rotate_discontinuous (first1, last1, d1, first2, last2, d2);
  if (d1 <= d2) rotate_discontinuous (std::next (first2, d2 - d1), last2, d1, first3, last3, d3);
  else
    {
...
bool
combine_discontinuous (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, BidirIter first2, BidirIter last2, typename std::iterator_traits<BidirIter>::difference_type d2, Function &f, typename std::iterator_traits<BidirIter>::difference_type d = 0)
{
  ...
  if (d1 == 1)
  ...
  else
    {
      ...
      for (D d22 = d2; i2 != last2; ++i2, --d22)
        {
          if (combine_discontinuous (f1p, last1, d1 - 1, i2, last2, d22, f, d + 1)) return true;
          swap (*first1, *i2);
        }
    }
  if (f ()) return true;
...
public:
  call_combine_discontinuous (BidirIter first1, BidirIter last1, D d1, BidirIter first2, BidirIter last2, D d2, Function &f) : f_ (f), first1_ (first1), last1_ (last1), d1_ (d1), first2_ (first2), last2_ (last2), d2_ (d2) {}
...
combine_discontinuous3_ (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, BidirIter first2, BidirIter last2, typename std::iterator_traits<BidirIter>::difference_type d2, BidirIter first3, BidirIter last3, typename std::iterator_traits<BidirIter>::difference_type d3, Function &f, typename std::iterator_traits<BidirIter>::difference_type d = 0)
{
  ...
  if (d1 == 1)
  ...
  else
    {
      ...
      for (D d22 = d2; i2 != last2; ++i2, --d22)
        {
          if (combine_discontinuous3_ (f1p, last1, d1 - 1, i2, last2, d22, first3, last3, d3, f, d + 1)) return true;
          swap (*first1, *i2);
        }
      i2 = first3;
      for (D d22 = d3; i2 != last3; ++i2, --d22)
        {
          if (combine_discontinuous (f1p, last1, d1 - 1, i2, last3, d22, f, d + 1)) return true;
          swap (*first1, *i2);
        }
    }
  if (f ()) return true;
  ...
  if (d != 0)
    {
      if (d2 > 1) rotate_discontinuous3 (first1, last1, d1, std::next (first2), last2, d2 - 1, first3, last3, d3);
      else
...
template <class BidirIter, class Function>
bool
permute_ (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, Function &f)
{
  using std::swap;
  switch (d1)
    {
  ...
  for (BidirIter p = fp1; p != last1; ++p)
    {
      if (permute_ (fp1, last1, d1 - 1, f)) return true;
...
template <class BidirIter, class Function>
bool
permute (BidirIter first1, BidirIter last1, typename std::iterator_traits<BidirIter>::difference_type d1, Function &f)
{
  using std::swap;
  switch (d1)
    {
...
{
  if (d1 < Int (0) || d2 < Int (0)) throw std::invalid_argument ("expected non-negative argument");
}
...
count_each_combination (UInt d1, UInt d2)
{
  ...
  if (d1 == UInt ()) return 1;
  if (d1 > std::numeric_limits<UInt>::max () - d2) throw std::overflow_error ("overflow in count_each_combination");
  UInt n = d1 + d2;
  UInt r = n;
  --n;
  for (UInt k = UInt (2); k <= d1; ++k, --n)
    {
      ...
      r /= g;
      UInt t = n / (k / g);
      if (r > std::numeric_limits<UInt>::max () / t) throw std::overflow_error ("overflow in count_each_combination");
      r *= t;
    }
  return r;
}
...
count_each_permutation (UInt d1, UInt d2)
{
  // return (d1+d2)!/d2!
  check_non_negative (d1, d2);
  if (d1 > std::numeric_limits<UInt>::max () - d2) throw std::overflow_error ("overflow in count_each_permutation");
  UInt n = d1 + d2;
  UInt r (1);
  for (; n > d2; --n)
    {
      if (r > std::numeric_limits<UInt>::max () / n) throw std::overflow_error ("overflow in count_each_permutation");
      r *= n;
    }
  return r;
}
...
template <class Function, class BidirIter> class circular_permutation
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
template <class BidirIter, class Function>
Function
for_each_circular_permutation (BidirIter first, BidirIter mid, BidirIter last, Function f)
{
  ...
  if (d1 <= d2)
    {
      ...
      for (--d1; d1 > UInt (1); --d1)
        {
          if (r > std::numeric_limits<UInt>::max () / d1) throw std::overflow_error ("overflow in count_each_circular_permutation");
          r *= d1;
        }
    }
  else
    { // functionally equivalent but faster algorithm
      if (d1 > std::numeric_limits<UInt>::max () - d2) throw std::overflow_error ("overflow in count_each_circular_permutation");
      UInt n = d1 + d2;
      r = 1;
      for (; n > d1; --n)
        {
          if (r > std::numeric_limits<UInt>::max () / n) throw std::overflow_error ("overflow in count_each_circular_permutation");
          r *= n;
        }
      for (--n; n > d2; --n)
        {
...
template <class Function, class Size> class reversible_permutation
{
  Function f_;
  Size s_;
...
public:
  ...
  template <class BidirIter> bool operator() (BidirIter first, BidirIter last);
};

// rev1 looks like call_permute
template <class Function, class BidirIter> class rev1
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
template <class Function, class BidirIter> class rev2
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
public:
  rev2 (Function f, BidirIter first1, BidirIter last1, D d1, BidirIter first2, BidirIter last2, D d2) : f_ (f), first1_ (first1), last1_ (last1), d1_ (d1), first2_ (first2), last2_ (last2), d2_ (d2) {}
...
template <class Function, class BidirIter> class rev3
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
public:
  rev3 (Function f, BidirIter first1, BidirIter last1, D d1, BidirIter first2, BidirIter last2, D d2, BidirIter first3, BidirIter last3, D d3) : f_ (f), first1_ (first1), last1_ (last1), d1_ (d1), first2_ (first2), last2_ (last2), d2_ (d2), first3_ (first3), last3_ (last3), d3_ (d3) {}
...
template <class BidirIter>
bool
reversible_permutation<Function, Size>::operator() (BidirIter first, BidirIter last)
{
  ...
  for (Size i = 1; i < s2; ++i, ++am1, ++a, ++ap1)
    {
      ...
      if (combine_discontinuous (first, a, i, ap1, last, s_ - i - 1, f2)) return true;
    }
  ...
  //     the middle.
  else
    {
      ...
      if (combine_discontinuous (bp1, a, s2 - 1, ap1, last, s_ - s2 - 1, f2)) return true;
      ...
      for (Size i = 1; i < s2 - 1; ++i, ++bm1, ++b, ++bp1)
        {
          ...
          if (combine_discontinuous3 (first, b, i, bp1, a, s2 - i - 1, ap1, last, s_ - s2 - 1, f3)) return true;
        }
      ...
      if (combine_discontinuous (first, b, s2 - 1, ap1, last, s_ - s2 - 1, f21)) return true;
...
template <class BidirIter, class Function>
Function
for_each_reversible_permutation (BidirIter first, BidirIter mid, BidirIter last, Function f)
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
count_each_reversible_permutation (UInt d1, UInt d2)
{
  ...
  check_non_negative (d1, d2);
  if (d1 > std::numeric_limits<UInt>::max () - d2) throw std::overflow_error ("overflow in count_each_reversible_permutation");
  UInt n = d1 + d2;
  UInt r (1);
  if (d1 > UInt (1))
    {
      ...
      if (r > std::numeric_limits<UInt>::max () / t) throw std::overflow_error ("overflow in count_each_reversible_permutation");
      r *= t;
      --n;
    }
  for (; n > d2; --n)
    {
      if (r > std::numeric_limits<UInt>::max () / n) throw std::overflow_error ("overflow in count_each_reversible_permutation");
      r *= n;
    }
  return r;
}
...
template <class Function, class BidirIter> class reverse_circular_permutation
{
  typedef typename std::iterator_traits<BidirIter>::difference_type D;
...
template <class BidirIter, class Function>
Function
for_each_reversible_circular_permutation (BidirIter first, BidirIter mid, BidirIter last, Function f)
{
  ...
  if (d1 > UInt (3))
    {
      for (--d1; d1 > UInt (2); --d1)
        {
          if (r > std::numeric_limits<UInt>::max () / d1) throw std::overflow_error ("overflow in count_each_reversible_circular_permutation");
          r *= d1;
        }
    }
  return r;
